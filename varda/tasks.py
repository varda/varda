...
def read_observations(observations, filetype='vcf'):
    ...
    for record in reader:
        ...
        # Todo: Use genotype likelihoods (GL or PL) if we don't have GT, or if
        #     the VCF was generated by samtools (Heng Li says [1] not to trust
        #     the GT values but the PL values).
        #
        ...
        #         range(len(record.ALT) + 1), ploidy), key=lambda g: g[::-1])
        #
        # best_gl = min((-sample.data.GL[i], i) for i in range(len(genotypes)))[1]
        # best_pl = min((sample.data.PL[i], i) for i in range(len(genotypes)))[1]
        #
        # gt_from_gl = genotypes[best_gl]
        # gt_from_pl = genotypes[best_pl]
